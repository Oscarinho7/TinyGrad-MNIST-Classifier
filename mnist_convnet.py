...
if __name__ == "__main__":
    ...
    def train_step():
        ...
        x = normalize(geometric_transform(X_train[samples], angle_deg, scale, shift_x, shift_y, SAMPLING)).reshape(B, 1, 28, 28)
        opt.zero_grad()
    ...
    for i in (t := trange(STEPS)):
        loss = train_step()
        if i % 10 == 9:
            acc = (model(normalize(X_test).reshape(-1, 1, 28, 28)).argmax(1) == Y_test).mean().item() * 100
            if acc > best_acc:
    ...
    # CLEAN
    for f in glob.glob(str(dir_name